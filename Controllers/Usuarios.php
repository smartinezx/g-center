...
class Usuarios extends Controller
{
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $data[$i]['accion'] = '<div>
                <button class="btn btn-primary" type="button" onclick="btnEditarUser(' . $data[$i]['id'] . ');"><i class="fas fa-pencil-alt"></i></button>
    ...
    public function newUser()
    {
        ...
        if (empty($usuario) || empty($nombre) || empty($correo) || empty($telefono)) {
            ...
            if (!empty($clave) && !empty($confirmar)) {
                if ($clave != $confirmar) {
                    ...
                    $data = $this->model->registrarNewUser($usuario, $nombre, $correo, $telefono, $hash);
                    if ($data == "ok") {
        ...
        $response = json_encode($msg, JSON_UNESCAPED_UNICODE);
