...
class Planes extends Controller
{
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $data[$i]['accion'] = '<div>
            <button class="btn btn-primary" type="button" onclick="btnEditarPlan(' . $data[$i]['id'] . ');"><i class="fas fa-pencil-alt"></i></button>
            <button class="btn btn-danger" type="button" onclick="btnEliminarPlan(' . $data[$i]['id'] . ');"><i class="fas fa-trash-alt"></i></button>
        </div>';
        }
        ...
        if (empty($nombre) || empty($descripcion) || empty($precio_plan)) {
            ...
            if ($id == "") {
                ...
                if ($data == "ok") {
                    ...
                    $msg = array('msg' => 'Membresia registrada con éxito', 'icono' => 'success');
                } else if ($data == "existe") {
                    $msg = array('msg' => 'La membresia ya existe', 'icono' => 'warning');
                } else {
                    $msg = array('msg' => 'Error al registrar membresia', 'icono' => 'error');
                }
            } else {
                ...
                if ($data == "modificado") {
                    ...
                    $msg = array('msg' => 'Membresia modificada', 'icono' => 'success');
                } else {
                    $msg = array('msg' => 'Error al modificar membresia', 'icono' => 'error');
                }
            }
        }
    ...
    public function eliminar($id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Membresia dada de baja', 'icono' => 'success');
        } else {
    ...
    public function desactivar($id)
    {
        ...
        if ($data == 'ok') {
            $msg = array('msg' => 'Membresia desactivada con éxito', 'icono' => 'success');
        }else{
            $msg = array('msg' => 'Error al desactivar membresia', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }
