...
class Clientes extends Controller
{
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $data[$i]['accion'] = '<div>
                <button class="btn btn-sm btn-primary editBtn" type="button" onclick="btnEditarCli(' . $data[$i]['id'] . ');"><i class="fas fa-pencil-alt"></i></button>
                <button class="btn btn-sm btn-danger" type="button" onclick="btnEliminarCli(' . $data[$i]['id'] . ');"><i class="fas fa-trash-alt"></i></button>
            </div>';
        }
    ...
    public function listar_pagos()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $data[$i]['ver'] = '<a class="btn btn-danger" href="' . base_url . 'clientes/pdfPago/' . $data[$i]['id'] . '" target="_blank"><i class="fas fa-file-pdf"></i></a>';
        }
        echo json_encode($data, JSON_UNESCAPED_UNICODE);
        die();
    }
    ...
    public function listar_plan_clientes()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            if ($data[$i]['estado'] == 1) {
                $data[$i]['estado'] = '<span class="badge bg-success">Activo</span>';
                $data[$i]['accion'] = '<div>
                    <button class="btn btn-success" type="button" onclick="pagoPlan(' . $data[$i]['id'] . ');"><i class="fas fa-dollar-sign"></i></button>
                    <a class="btn btn-primary" href="' . base_url . 'clientes/verpagos/' . $data[$i]['id_cliente'] . '"><i class="fas fa-eye"></i></a>
                ...
                $data[$i]['accion'] = '<a class="btn btn-primary" href="' . base_url . 'clientes/verpagos/' . $data[$i]['id_cliente'] . '"><i class="fas fa-eye"></i></a>';
            }
        }
        echo json_encode($data, JSON_UNESCAPED_UNICODE);
        die();
    ...
    public function pdfPago($id)
    {
        ...
        $pdf->Image('Assets/images/g-center_logo.png', 5, 5, 20, 20);
